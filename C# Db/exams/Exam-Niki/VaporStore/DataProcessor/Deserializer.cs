    ...
    public static class Deserializer
	{
		public static string ImportGames(VaporStoreDbContext context, string jsonString)
		{
            ...
            foreach (var jsonGame in games)
            {
				...
				// Valid data
				// СЪЗДАВА ЖАНР ИЛИ ГО ТЪРСИ В БАЗАТА
				...
				var game = new Game
				{
					...
					// ПИШЕМ ВАЛЬО ЗАЩОТО СМЕ ГО НАПРАВИЛИ НЪЛЪБЪЛ
					ReleaseDate = jsonGame.ReleaseDate.Value,
				};
				// ЗА ВСЕКИ ЕДИН ОТ ТАГОВЕТЕ ТЪРСИМ ГО В БАЗАТА ИЛИ ГО СЪЗДАВАМЕ
				// И ГО ДОБАВЯМЕ КЪМ ИГРАТА
				// ПРАВИМ ГО ТАКА ЗАЩОТО ТАГОВЕТЕ СА МАСИВ ОТ СТРИНГОВЕ ВИЖ ГАМЕ.ДЖЕЙСОН
		...
		public static string ImportUsers(VaporStoreDbContext context, string jsonString)
		{
            ...
            {
				//АКО НЕ Е ВАЛИДЕН
				// !jsonUser.Cards.All(IsValid) ПРОВЕРЯВА ДАЛИ ВСИЧКИ КАРТИ СА ВАЛИДНИ
				...
				// АКО Е ВАЛИДЕН ТАКА ГО СЪЗДАВАМЕ
				var user = new User
				{
					Age = jsonUser.Age,
		...
		public static string ImportPurchases(VaporStoreDbContext context, string xmlString)
		{
			...
			var xmlSerializer = new XmlSerializer(
				typeof(PurchaseXmlInputModel[]), // ТУК НЕМОЖЕМ ДА ИЗПОЛЗВАМЕ ieNUMERUBLE И ЗАТОВА И ЗАТОВА ИЗПОЛЗВАМЕ МАСИВ
				new XmlRootAttribute("Purchases"));// ТУК СЛАГАМЕ РООТ ЕЛЕМЕНТА ТОВА Е ЗАГЛАВИЕТО
