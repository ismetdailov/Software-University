...
namespace Easter.Core
{
    public class Controller : IController
    {
        ...
        public string ColorEgg(string eggName)
        {
            ...
            {
                IBunny cureentBunny = suitableBunnies.First();
                while (true)
                {
                    if (cureentBunny.Energy==0 || cureentBunny.Dyes.All(x=>x.IsFinished()))
                    {
                        suitableBunnies.Remove(cureentBunny);
                        break;
                    }
                    workshop.Color(egg, cureentBunny);
                    if (egg.IsDone())
                    {
                        break;
                    }
